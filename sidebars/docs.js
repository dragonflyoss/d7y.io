module.exports = {
  docs: [
    {
      type: 'doc',
      id: 'introduction',
    },
    {
      type: 'category',
      label: 'Getting Started',
      items: [
        {
          type: 'category',
          label: 'Quick Start',
          link: {
            type: 'doc',
            id: 'getting-started/quick-start',
          },
          items: [
            {
              type: 'category',
              label: 'Kubernetes',
              items: [
                'getting-started/quick-start/kubernetes/lightweight-deployment',
                'getting-started/quick-start/kubernetes/deployment-with-manager',
              ],
            },
            {
              type: 'category',
              label: 'Multi-cluster Kubernetes',
              items: [
                'getting-started/quick-start/multi-cluster-kubernetes/lightweight-deployment',
                'getting-started/quick-start/multi-cluster-kubernetes/deployment-with-manager',
              ],
            },
          ],
        },
        {
          type: 'category',
          label: 'Installation',
          items: ['getting-started/installation/helm-charts', 'getting-started/installation/binaries'],
        },
      ],
    },
    {
      type: 'category',
      label: 'Operations',
      items: [
        {
          type: 'category',
          label: 'Deployment',
          items: ['operations/deployment/deployment-models', 'operations/deployment/deployment-best-practices'],
        },
        {
          type: 'category',
          label: 'Integrations',
          items: [
            {
              type: 'category',
              label: 'Container Runtime',
              items: [
                'operations/integrations/container-runtime/containerd',
                'operations/integrations/container-runtime/singularity',
                'operations/integrations/container-runtime/docker',
                'operations/integrations/container-runtime/cri-o',
                'operations/integrations/container-runtime/podman',
                'operations/integrations/container-runtime/nydus',
                'operations/integrations/container-runtime/stargz',
              ],
            },
            'operations/integrations/harbor',
            'operations/integrations/hugging-face',
            'operations/integrations/model-scope',
            'operations/integrations/opencsg',
            'operations/integrations/git-lfs',
            'operations/integrations/torchserve',
            'operations/integrations/triton-server',
            'operations/integrations/pip',
            'operations/integrations/upgrade',
          ],
        },
        {
          type: 'category',
          label: 'Observability',
          items: [
            {
              type: 'autogenerated',
              dirName: 'operations/observability',
            },
          ],
        },
        {
          type: 'category',
          label: 'Security',
          link: {
            type: 'doc',
            id: 'operations/security',
          },
          items: [
            {
              type: 'autogenerated',
              dirName: 'operations/security',
            },
          ],
        },
      ],
    },
    {
      type: 'category',
      label: 'Architecture',
      link: {
        type: 'doc',
        id: 'operations/architecture/architecture',
      },
      items: [
        {
          type: 'category',
          label: 'Components',
          items: [
            'operations/architecture/components/manager',
            'operations/architecture/components/scheduler',
            'operations/architecture/components/client',
          ],
        },
      ],
    },
    {
      type: 'category',
      label: 'Concepts',
      items: [
        {
          type: 'autogenerated',
          dirName: 'concepts',
        },
      ],
    },
    {
      type: 'category',
      label: 'Reference',
      items: [
        {
          type: 'category',
          label: 'Configuration',
          items: [
            'reference/configuration/manager',
            'reference/configuration/scheduler',
            {
              type: 'category',
              label: 'Client',
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'reference/configuration/client',
                },
              ],
            },
          ],
        },
        {
          type: 'category',
          label: 'Commands',
          items: [
            'reference/commands/manager',
            'reference/commands/scheduler',
            {
              type: 'category',
              label: 'Client',
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'reference/commands/client',
                },
              ],
            },
          ],
        },
      ],
    },
    {
      type: 'category',
      label: 'Advanced Guides',
      items: [
        {
          type: 'category',
          label: 'Open API',
          items: [
            {
              type: 'autogenerated',
              dirName: 'advanced-guides/open-api',
            },
          ],
        },
        {
          type: 'category',
          label: 'Web Console',
          link: {
            type: 'doc',
            id: 'advanced-guides/web-console',
          },
          items: [
            'advanced-guides/web-console/signin',
            'advanced-guides/web-console/signup',
            'advanced-guides/web-console/cluster',
            {
              type: 'category',
              label: 'Job',
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'advanced-guides/web-console/job',
                },
              ],
            },
            {
              type: 'category',
              label: 'Resource',
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'advanced-guides/web-console/resource',
                },
              ],
            },
            'advanced-guides/web-console/audit',
            'advanced-guides/web-console/gc',
            {
              type: 'category',
              label: 'Developer',
              items: [
                {
                  type: 'autogenerated',
                  dirName: 'advanced-guides/web-console/developer',
                },
              ],
            },
            'advanced-guides/web-console/user',
          ],
        },
        'advanced-guides/preheat',
        'advanced-guides/request-sdk',
        'advanced-guides/rate-limit',
        'advanced-guides/leech',
        'advanced-guides/blocklist',
        'advanced-guides/personal-access-tokens',
      ],
    },
    {
      type: 'category',
      label: 'Development Guides',
      items: [
        'development-guide/configure-development-environment',
        {
          type: 'category',
          label: 'Plugins',
          items: ['development-guide/plugins/out-of-tree-plugins', 'development-guide/plugins/in-tree-plugin'],
        },
        'development-guide/running-tests',
        'development-guide/system-design',
      ],
    },
    {
      type: 'category',
      label: 'Roadmap',
      items: [
        {
          type: 'autogenerated',
          dirName: 'roadmap',
        },
      ],
    },
    {
      type: 'doc',
      id: 'faq',
    },
  ],
};
