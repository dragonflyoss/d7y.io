...
- **Lightweight deployment with Redis**: Deploy Redis in addition to the lightweight
...
- **Deployment with Manager**: Deploy the Manager along with MySQL and Redis. It additionally
...
dynamic configuration from the local `dynconfig.yaml` file mounted as a ConfigMap, and clients
discover schedulers via the scheduler headless service. The local dynamic configuration can be
customized with the `scheduler.dynconfig`, `seedClient.dynconfig` and `client.dynconfig` values,
...
## Create Dragonfly cluster with Redis based on helm charts {#create-dragonfly-cluster-with-redis-based-on-helm-charts}
...
## Create Dragonfly cluster with Manager based on helm charts {#create-dragonfly-cluster-with-manager-based-on-helm-charts}
...
Use `dfctl` to preheat images without the Manager, or use the Open API and the web console
with the Manager, refer to [Preheat](../../advanced-guides/preheat.md).
